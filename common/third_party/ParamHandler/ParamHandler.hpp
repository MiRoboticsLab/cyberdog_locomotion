...
public:
    ...
    NodeType::value getType( const std::string& key ) {
        return config_[ key ].Type();
    }

    template < typename T > bool getVector( const std::string& key, std::vector< T >& vec_value ) {
        try {
    ...
    template < typename T > bool getVector( const std::string& category, const std::string& key, std::vector< T >& vec_value ) {
        try {
    ...
    template < typename T > bool get2DArray( const std::string& category, const std::string& key, std::vector< std::vector< T > >& vec_value ) {
        try {
    ...
    template < typename T > bool get2DArray( const std::string& key, std::vector< std::vector< T > >& vec_value ) {
        try {
    ...
    template < typename T > bool getValue( const std::string& key, T& T_value ) {
        try {
    ...
    template < typename T > bool getValue( const std::string& category, const std::string& key, T& T_value ) {
        try {
